    ...
    try {
      ...
      switch (mutation.action) {
        ...
        case 'UPDATE_TASK':
          ...
          if (payloadMap.containsKey('studentId'))
            data['student_id'] = payloadMap['studentId'];
          if (payloadMap.containsKey('title'))
            data['title'] = payloadMap['title'];
          if (payloadMap.containsKey('type')) data['type'] = payloadMap['type'];
          if (payloadMap.containsKey('dueAt'))
            data['due_at'] = payloadMap['dueAt'];
          if (payloadMap.containsKey('priority'))
            data['priority'] = payloadMap['priority'];
          if (payloadMap.containsKey('isCompleted'))
            data['is_completed'] = payloadMap['isCompleted'];
        ...
        case 'UPDATE_PARENT_PROFILE':
          final Map<String, dynamic> data = {'id': payloadMap['id']};
          if (payloadMap.containsKey('displayName'))
            data['display_name'] = payloadMap['displayName'];
          if (payloadMap.containsKey('email'))
            data['email'] = payloadMap['email'];
          if (payloadMap.containsKey('notifyOnTaskCompletion'))
            data['notify_on_task_completion'] =
                payloadMap['notifyOnTaskCompletion'];
          if (payloadMap.containsKey('notifyOnMoodDrop'))
            data['notify_on_mood_drop'] = payloadMap['notifyOnMoodDrop'];
          if (payloadMap.containsKey('canViewJournal'))
            data['can_view_journal'] = payloadMap['canViewJournal'];
